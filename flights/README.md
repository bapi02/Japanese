...
- 좌석등급: **이코노미 + 비즈니스를 같이** 조회. 칸마다 이코노미 가격 아래
  비즈니스 가격과 배수(`×3.2`)가 붙고, 상세 화면에서 두 등급을 눌러 비교합니다.
- 각 칸에 최저가 + 히트맵 색 + **비싼 이유**(연휴·성수기·잔여석·예약클래스·평균 대비,
  그리고 이코노미↔비즈니스 격차가 평소보다 좁으면 `비즈니스 가성비` 배지)
...
- Variables(선택): `AMADEUS_ENV`(기본 `production`), `FARE_DESTS`(기본 `NRT,HND,KIX,FUK,NGO`),
  `FARE_WEEKS`(기본 `6`), `FARE_CABINS`(기본 `ECONOMY,BUSINESS`)
...
호출 수 = `목적지 수 × 주 수 × 4(패턴) × 좌석등급 수`.
기본값은 5 × 6 × 4 × 2 = **240회/일** 입니다. 좌석등급이 둘이라 호출이 두 배이므로
기본 주 수를 6주로 잡아뒀습니다. 무료 쿼터에 여유가 있으면 `FARE_WEEKS` 를 올리고,
빠듯하면 `FARE_CABINS=ECONOMY` 로 줄이세요.
...
- 좌석등급마다 Amadeus 호출이 따로 나갑니다. 화면에서 `이코노미`나 `비즈니스`
  하나만 선택해 두면 라이브 갱신도 그 등급만 조회합니다.
...
- 예약클래스 등급 분류(이코노미 `Y/B/M…`, 비즈니스 `J/C/D/I/Z`)는 항공사 공통
  관행에 기댄 근사치이며, 정확한 운임규정은 항공사 공지가 우선입니다.
