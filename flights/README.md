...
- 좌석등급: **이코노미 + 비즈니스**. 이코노미는 자동 수집하고, 비즈니스는 보고 싶은
  ...
  온디맨드로 조회한 값은 기기에 **6시간 보존**되어 새로고침해도 한도를 다시 쓰지 않습니다.
...
키를 받으면 먼저 **스모크 테스트**로 1~2회만 써서 실응답이 코드와 맞는지 확인하세요:

```bash
SERPAPI_KEY=... node flights/scripts/smoke.mjs            # 호출 1회
SERPAPI_KEY=... node flights/scripts/smoke.mjs NRT --return --save=/tmp/raw.json  # 호출 2회 + 원본 저장
```

통과가 뜨면 Actions/Worker 를 켜도 됩니다. 실패하면 `--save` 로 저장한 원본을
보고 `lib/search.mjs` 의 정규화를 맞추면 됩니다.
...
- **출발 임박도** — D-21 이내
- **지난 수집 대비 변화** — 같은 조합을 직전 수집 때 얼마였는지 (±5% 이상일 때 배지).
  수집기가 조합별 최저가를 `data/history.json` 에도 쌓아둡니다(최근 30회).
- **이코노미↔비즈니스 배수** — 차액과 배수를 표시. `비즈니스 가성비` 판정(평소 배수와
  비교)은 두 등급 가격이 함께 있는 조합이 충분할 때만 뜹니다 — 비즈니스가 온디맨드인
  기본 설정에서는 표본이 적어 거의 나타나지 않습니다.
...
- 공휴일 표(`lib/holidays.mjs`)는 **2027년 말까지**입니다. 그 이후를 보게 되면
  화면과 수집 로그에 경고가 뜹니다 — 다음 해 공휴일을 표에 추가하세요.
